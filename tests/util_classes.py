...
class TestResults:
    ...
    def set_statistic_value(self):
        ...
        print(f'self.statistic_values: {self.statistic_values}')
        
    def sort_estimator_values(self):
        self.unsorted_estimator_values[:] = self.estimator_values[:]
        # Generate small random noise
        random_noise = np.random.uniform(0, 1e-10, size=self.B+1)
        array_with_noise = self.estimator_values[:] + random_noise
        # Get the indices that would sort the array
        self.sorted_indices = np.argsort(array_with_noise)
        # Use the indices to sort the array
        self.estimator_values[:] = self.estimator_values[self.sorted_indices]
        # Get the inverse permutation
        self.inverse_permutation = np.empty_like(self.sorted_indices)
        self.inverse_permutation[self.sorted_indices] = np.arange(self.B+1)
        
    def set_threshold(self, alpha):
        self.threshold_position = np.ceil((1-alpha)*(self.B+1)).astype('int')-1
        self.threshold_values = self.estimator_values[np.ceil((1-alpha)*(self.B+1)).astype('int')-1]
        print(f'self.threshold_values: {self.threshold_values}')
    ...
    def set_reject(self, alpha, reject_with_threshold=False):
        if reject_with_threshold:
            ...
            R = self.inverse_permutation[self.B] + 1
            p_alpha = min(1, max(0, R - (1 - alpha) * (self.B + 1)))
            self.rejects = 1 if random.random() < p_alpha else 0
